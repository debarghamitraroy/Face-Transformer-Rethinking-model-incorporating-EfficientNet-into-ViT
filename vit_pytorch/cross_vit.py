...
class ArcFace(nn.Module):
    ...
    def __init__(
        ...
        self.th: float = math.cos(math.pi - self.m)
...
class CrossTransformerBlock(nn.Module):
    def __init__(
        ...
        self.sm_attend_lg = ProjectInOut(
            sm_dim,
            lg_dim,
            Attention(
                lg_dim,
        ...
        self.lg_attend_sm = ProjectInOut(
            lg_dim,
            sm_dim,
            Attention(
                sm_dim,
    ...
    def forward(
        ...
        sm_cls: torch.Tensor,
        lg_cls: torch.Tensor,
        sm_patch_tokens: torch.Tensor,
        lg_patch_tokens: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        sm_cls = (
            self.sm_attend_lg(
                sm_cls,
                context=lg_patch_tokens,
                kv_include_self=True,
            )
            + sm_cls
        )
...
class CrossTransformer(nn.Module):
    ...
    def forward(
        self,
        sm_tokens: torch.Tensor,
        lg_tokens: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor]:
...
class MultiScaleEncoderBlock(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.sm_transformer = Transformer(
            dim=sm_dim,
            dropout=dropout,
            **sm_enc_params,
        )

        self.lg_transformer = Transformer(
            dim=lg_dim,
            dropout=dropout,
            **lg_enc_params,
        )
    ...
    def forward(
        ...
        sm_tokens = self.sm_transformer(sm_tokens)
        lg_tokens = self.lg_transformer(lg_tokens)
        sm_tokens, lg_tokens = self.cross_transformer(sm_tokens, lg_tokens)
        return sm_tokens, lg_tokens
        ...
        self.layers = nn.ModuleList(
            [
                MultiScaleEncoderBlock(
                    ...
                    sm_enc_params=sm_enc_params,
                    lg_enc_params=lg_enc_params,
                    dropout=dropout,
                )
                for _ in range(depth)
            ]
